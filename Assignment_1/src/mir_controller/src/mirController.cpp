...
/**
 * @brief 
 * Initialize target points that the mir robot has to visit inorder
 */
...
int main(int argc, char **argv){
  ...
  if(ros::ok()){
    ...
    //Reach straight arm configuration
    ...
    for(MobileRobot target_pos : targetPoints){  //reach all the target area in sequence
      ...
      //To be sure that topic data are up to date it is better to wait some seconds
      sleep(2);
